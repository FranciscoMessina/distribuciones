...
import { DistribucionWeibull } from "@/lib/distribuciones"
import type { ParamsWeibull } from "@/lib/distribuciones/weibull"
import { modaWeibull } from "@/lib/distribuciones/weibull"

type Props = {
  params: ParamsWeibull
  x?: number | null
  modo?: "izq" | "der"
}
...
export function WeibullChart({ params, x, modo = "izq" }: Props) {
  const { beta, omega } = params
  const tieneX = x != null && !isNaN(x) && x >= 0

  const data = useMemo(() => {
    // p99 como límite superior
    const p99 = beta * Math.pow(-Math.log(0.01), 1 / omega)
    const xMax = Math.max(p99, tieneX ? x! : 0) * 1.05
    const xMin = 0

    return Array.from({ length: PUNTOS + 1 }, (_, i) => {
      const xv = xMin + (i / PUNTOS) * (xMax - xMin)
      const pdf = DistribucionWeibull.pdf(xv, params)
      return {
        x: xv,
